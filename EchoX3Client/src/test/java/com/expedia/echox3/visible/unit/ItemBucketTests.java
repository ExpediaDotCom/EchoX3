...
import com.expedia.echox3.internal.store.cache.LocalObjectBucket;
...
public class ItemBucketTests extends AbstractTestTools
{
	...
	static
	{
		...
		try
		{
			bytes1 = BasicSerial.toBytes(ItemBucketTests.class.getSimpleName(), QUICK_FOX_TEXT + "1");
			bytes2 = BasicSerial.toBytes(ItemBucketTests.class.getSimpleName(), QUICK_FOX_TEXT + "2");
			bytes3 = BasicSerial.toBytes(ItemBucketTests.class.getSimpleName(), QUICK_FOX_TEXT + "3");
		}
	...
	public void testSimple() throws BasicException
	{
		...
		LocalObjectBucket bucket				= new LocalObjectBucket(configuration, trackingObjects, 5);
		assertEquals(0, bucket.getItemCount());
...
//		TestObjectFactory			factory				= new TestObjectFactory();
		...
		ObjectWrapper				wrapper1			= bucket.getEntry(KEY_1, wrapper);
		ObjectWrapper				wrapper2			= bucket.getEntry(KEY_2, wrapper);
		ObjectWrapper				wrapper3			= bucket.getEntry(KEY_3, wrapper);
		assertEquals(3, bucket.getItemCount());

		getLogger().info(BasicEvent.EVENT_DEBUG, "bin = %s", bucket.toString());
		...
		bucket.deleteEntry(KEY_2);
		assertEquals(2, bucket.getItemCount());

		ObjectWrapper				wrapper2b			= bucket.getEntry(KEY_2, null);
		...
		bucket.doMaintenance(maintenanceRequest, maintenanceResponse);
	}
}
